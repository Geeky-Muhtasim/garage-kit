    ...
    <div className="flex flex-col gap-3">
          ...
          {categories.slice(0, 15).map(cat => {
            ...
            return (
              <label key={cat.name} className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-xs text-txt-2 flex-1 truncate">
                  {toTitleCase(cat.name)}
                </span>
                <span className="text-[10px] text-txt-3 font-mono">{cat.count}</span>
              </label>
            );
          })}
        </div>
          ...
          {brands.slice(0, 10).map(b => {
            ...
            return (
              <label key={b.name} className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-xs text-txt-2 flex-1 truncate">{b.name}</span>
                <span className="text-[10px] text-txt-3 font-mono">{b.count}</span>
              </label>
            );
          })}
        </div>
      ...
      <FilterBlock title="Price range">
        <div className="flex gap-1.5">
          <input
            ...
            className="w-full bg-surf2 border-[0.5px] border-bdr rounded text-xs text-txt px-2 py-2 font-mono outline-none focus:border-amber"
          />
      ...
      >
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          />
          <span className="text-xs text-txt-2">In stock only</span>
        </label>
      </FilterBlock>
    </div>
  );
